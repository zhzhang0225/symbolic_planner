            ...
            int moveActionIndex, int moveToTableActionIndex, int*** plan, int* planlength) 
{
	...
	*planlength = 0;
    ...
    int num_of_obj = numofblocks + numoftriangles;
    vector<int> obj_info (num_of_obj, 0);

    for (int i = 0; i < numoftriangles; i++) obj_info[trianglesV[i]] = 1; 

    // initialize task object
    TaskPlanner blockMover(obj_info, TableIndex);

	blockMover.setAction(moveActionIndex, moveToTableActionIndex);

    // define start/goal state
	blockMover.setInitState(onV_start, onV_start_length, clearV_start, numofclear_start);
	blockMover.setGoalState(onV_goal, onV_goal_length, clearV_goal, numofclear_goal);

	//generate plan 
	vector<vector<int>> plan_mat;
	blockMover.generatePlan(plan_mat);
    
    //allocate memory and set the return plan

    int numofsteps = plan_mat.size();    
    *plan = (int**) malloc(numofsteps*sizeof(int*));
    for (int i = 0; i < numofsteps; i++){
        (*plan)[i] = (int*) malloc(4*sizeof(int)); 

        (*plan)[i][0] = plan_mat[numofsteps - i - 1][0];
        (*plan)[i][1] = plan_mat[numofsteps - i - 1][1];
        (*plan)[i][2] = plan_mat[numofsteps - i - 1][2];
        (*plan)[i][3] = plan_mat[numofsteps - i - 1][3];                         
